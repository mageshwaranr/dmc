...
public class DependencyInstanceResourceIntegrationTest {
    ...
    String REF_RULE_NAME = "Test.TransactionRule";
    ...
    private String acctAndProdAndTxRule = new StringBuilder("Matcher.")
            .append("event('ACCOUNTS').set('BD').fromHeader('date')")
            .append(".and().event('PRODUCTS').set('BD').fromHeader('date')")
            .append(".and().event('TRANSACTIONS').set('BD').fromHeader('date')")
    ...
    private String acctAndProdAndTxAndPc2PremiseRule = new StringBuilder("Matcher.")
            ...
            .append(".and().is('TRANSACTIONS','BD').eq('PRODUCTS','BD')")
    ...
    public void test01CreateRules() {
        String url = "http://www.mocky.io/v2/5d8b05dd3500005c00d46b5e";
        RuleDefinition rule = restService.registerRule(createRule(REF_RULE_NAME, acctAndProdAndTxRule, url));
        verifyRuleAndSingleCallback(rule);

        List<CallbackDefinition> callbackList;
        CallbackDefinition callback;
        rule = restService.registerRule(createRule("Test.ReferenceDataRule", acctAndProdRule, url));
        assertNotNull(rule.getId());
        callback = restService.addCallback(rule.getId(), rule.getName(), createCallback(CALLBACK_NAME, "http://localhost:12345"));
        ...
        rule = restService.registerRule(createRule("Test.LoadProductsRule", prodOnlyRule, url));
        verifyRuleAndSingleCallback(rule);
...
//    @Test
    public void test04DeleteRule(){
        ...
        assertEquals(2,ruleList.size()); //6-1
    }
